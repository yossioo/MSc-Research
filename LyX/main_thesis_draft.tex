...
serve as foundation for every analytical approach of robotics study.\citet{Lozano-Perez1983}
for the introduction of Configuration Space use which contributed
in vast majority of fields. Use of the Configuration Space representation
has been a major interest for the simultaneous multi-body manipulation,
yet because of high computation cost and lack of explicit analytical
formulations was not fully incorporated in this research.
...
contacts for frictional, frictionless, hard or soft contacts\citet{Murray1994}.\citet{Rimon1995}
proposed bounds on number of frictionless fingers for 2D polygonal
...
the stability of grasps,\citet{Ferrari1992} introducing quality measures
based on wrenches,\citet{Roa2015} surveying several methods of grasp
quality measure, while\citet{Lin1997} extend the measures for complaint
grasps.
...
\begin{figure}[tbh]
\noindent \centering{}\includegraphics[width=0.25\textwidth]{\string"images/object w contact\string".pdf}\caption{\label{fig:Coordinate-frames}Coordinate frames.}
\end{figure}
...
\begin{figure}[tbh]
\noindent \centering{}\includegraphics{\string"images/friction cone\string".pdf}\caption{\label{fig:Friction-cone}Friction cone.}
\end{figure}
...
\end{bmatrix}
\end{equation}
where$R_{c}$ represents\z{WHAT?} Grasp map for this example is given
by:
\begin{equation}
...
placement is proposed.\z{The idea can be expanded for various tasks, such as home assistant robot that brings several items in one run.
...
Let $C=\left\{ c_{i}\right\} $ be a set of contact descriptions between these polygons. }Let$\hat{n}_{i}^{j}$ denote the inward normal direction of the boundary
...
\emph{The existence of the solution depends on the rank of the$G$
matrix. Since there are more columns than rows (and therefore there
is no unique solution) we need to ensure that the rows are linearly
independent.}

\emph{Since the matrix is built with blocks, we can see that there
are no (and cannot be) 2 linearly dependent block rows.}

\emph{Assuming that block rows i and j are linearly dependent. In
particular, first$G$-blocks (that corresponds to set of forces applied
by fingers) on these rows are at the same columns. That means that
same forces act on different bodies, while acting at the same point.
Which is impossible, since by definition each finger contacts one
body only.}

\emph{Therefore, if given matrix has full rank, there exists a non-trivial
(but not unique) set of forces that will keep the system in equilibrium.}

\emph{3 Frictionless contacts that intersecting at one point.}

\emph{Direction of contacts should}
...
\section{Algorithms for finger placement\label{sec:Algorithms-for-finger}}
...
Assessment of such cases is discussed in\ref{sec:Alg-for-multiO-eval}.
...
\section{Algorithm for multi object grasp evaluation\label{sec:Alg-for-multiO-eval}}
...
contact positions of added fingers can be altered in a regions that
was found for that contact (see above (what, the regions?) maybe \#ADDFIG)

\section{Algorithm for objects placement}
...
convex cone can be seen as infinite``corridor'', formed by the contact
region swept along the contacts' normal direction. The interaction
can be considered useful in cases where some other 2 different edges
provide normal directions that are crossing in the parallel contacts'``corridor''.
Simple example would be a case with a vertex inside of the``corridor'',
where normal directions of 2 adjacent edges induce opposite wrenches
about the edge with two contacts. This requires at least 2 additional
external contacts for each object to complete the fixture.
...
\par\end{centering}
\noindent \centering{}\caption{Inner vertex.\label{fig:Inner-vertex.}}
\end{figure}
...
2 normals. Example of such configuration is shown in\figref{In-v-b}.
...
Algorithm 1:
\begin{enumerate}
\item Scan the objects
\item If There exist several object with concave vertices -
\begin{enumerate}
\item select one with largest angle to be base object or the one with longest
edges next to the concave vertex
\end{enumerate}
\item ElseIf 1 such object
\begin{enumerate}
\item - select it to be base
\end{enumerate}
\item Else (no concave angles)
\begin{enumerate}
\item assess angles sum == 180/360 deg
\begin{enumerate}
\item if such exist - they can be stacked together. Maybe some of them will
form a
\end{enumerate}
\item Find 2 objects with longest edges, align 1 vertex of the object with
2nd longest edge to the middle of the another object's longest edge
and the edges are collinear
\end{enumerate}
\item Now there exists a concavity - either from one object
\item Find objects with smallest angles (smaller than the concavity angle),
sort\textbf{descending}/ascending ?
\begin{enumerate}
\item select the object and stick it in the concavity.
\item continue along the list or until an object cannot fill in. Use one
additional item to stick not until the end - form edge+vertex contact.
\begin{itemize}
\item Optional -- each time reassess the configurations, and find concavities.
Check whether some of them give longer common edge segements, and
stick objects there
\end{itemize}
\end{enumerate}
\item Items that did not fit in any concavity v2v type -- check e-v type
\item Place fingers on each item. (Can we stick more items to bound these?)
\end{enumerate}
\z{CONTINUE HERE}

\z{See whether this is relevant:}

This way the number of objects that are stuffed inside the concavity
is maximized. Optimally, either all objects are aligned with their
sharpest vertices and stacked inside the concavity, or the concavity
is filled with several objects.

Another options is if some objects is quite small, and being stacked
in the concavity form 2 additional inner vertices of the combined
super-object.

If there are no concave objects, a pair of objects can be stacked
to form a concavity. In this case, an object with longest edge is
selected to be a base and another object with an edge that is shorter
than the first one and capable to be immobilized with 2 additional
fingers apart from 2 parallel inter-object contacts is stacked so
the edges are contiguous are in contact. One of the vertices of the
shorter edge will be a concavity, and from here the algorithm for
the concavities can be applied.

Polygonal objects analyzed and selected in the following order: Objects
with inner vertices are searched for and selected to be base objects,
which other objects are going to lean on. Inner vertex can provide
beneficial inter-object contacts for one or more additional polygonal
objects.

If several objects with inner vertices exists they can be stacked
in non exclusive way and pro

VARIANT A: No concave objects: select two with longest edges, and
stack them together. Form a concavity, insert small objects from smallest
to largest.

VARIANT B: Some concave object(s). Starting with one, proceeding with
stacking other convex, and then other concave to form continuity.
...
\section{Rearranging the objects}
...
The performance of the algorithms was tested in experimental setups
in the laboratory.

The objects are known and only the location of objects is obtained
via RGBD camera. The robot is equipped with an end-effector designed
according to the algorithm's output. Robotic arm used is ROBOTIS Manipulator
H, controlled by laptop with Robot Operating System (ROS) hosted in
Ubuntu.
